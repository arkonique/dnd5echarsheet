...
export async function encodePassword(string) {
  ...
  const serverbits = await postData('http://localhost/dnd_api/index.php/user/bits');
  const newpw = serverbits+buffer+clientBits; 
  return [newpw,salt]
}

export async function verifyUser(username,password) {
  const userlist = await getData('http://localhost/dnd_api/index.php/user/list');
  const n = checkOcurrence(username,userlist);  
  if (n===1) {
    const salt = (await postData('http://localhost/dnd_api/index.php/user/salt',{u:username}))[0].salt;
    const buffer = (await pbkdf2Async(password, salt, 100000, 502,  'sha512')).toString('hex');
    const check = (await postData('http://localhost/dnd_api/index.php/user/get',{u:username,p:buffer}))
